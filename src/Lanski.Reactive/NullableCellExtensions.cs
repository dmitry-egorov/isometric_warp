...
namespace Lanski.Reactive
{
    public static class NullableCellExtensions
    {
        ...
        public static ICell<TOut> Select_Cell_Or_Single_Default<TIn, TOut>(this Possible<TIn> possible, Func<TIn, ICell<TOut>> selector) => possible.Select(selector).Value_Or(new Cell<TOut>(default(TOut)));
        
        public static ICell<T> Cell_Or_Single_Default<T>(this Possible<ICell<T>> possible_cell) where T : struct => possible_cell.Value_Or(new Cell<T>(default(T)));
