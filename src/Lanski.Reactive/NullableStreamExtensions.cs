...
namespace Lanski.Reactive
{
    ...
    public static class NullableStreamExtensions
    {
        ...
        public static IStream<TOut> SelectMany_Or_Single_Default<TIn, TOut>(this IStream<Possible<TIn>> stream, Func<TIn, IStream<TOut>> selector) => stream.SelectMany(p => p.Select(selector).Value_Or(CellsCache<TOut>.Default));
        public static IStream<TOut> SelectMany_Or_Empty<TIn, TOut>(this IStream<Possible<TIn>> stream, Func<TIn, IStream<TOut>> selector) => stream.SelectMany(p => p.Select(selector).Value_Or(StreamsCache<TOut>.Empty));
    }
}